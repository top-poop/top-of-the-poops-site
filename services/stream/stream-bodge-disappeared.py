    ...
    with psycopg2.connect(host=db_host, database="gis", user="docker", password="docker",
        ...
        for company in companies:
            ...
            if gone_away:
                ...
                fakes = [FeatureRecord(id=cso_id, company=company, status=EventType.Stop.value, statusStart=gone_away_date,
                                       latestEventStart=gone_away_date, latestEventEnd=gone_away_date,
                                       lastUpdated=gone_away_date, lat=0, lon=0, receivingWater='None') for cso_id in
                         gone_away]

                file_ref = database.create_file(company=company,
                                                file_time=most_recent_file.file_time + datetime.timedelta(
                                                    milliseconds=1))
