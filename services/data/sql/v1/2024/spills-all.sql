with cons as (select *
     ...
     locality_agg AS (SELECT gr.pcon24nm,
                             gr.grid_reference,
     ...
     agg as (select cons.pcon24nm                           as constituency,
             ...
             from cons
                      ...
                      left join senedd_cons sc on cons.pcon24nm = sc.pcon24nm
                      left join senedd_final_2026 s on sc.ogc_fid = s.ogc_fid
             where cons.pcon24nm is not null
             group by cons.pcon24nm, senedd, company_name, discharge_site_name, receiving_water, lat, lon, localities)
select *
from agg
