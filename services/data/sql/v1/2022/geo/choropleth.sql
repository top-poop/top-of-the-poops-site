with reports_last as (select pcon20nm                                as constituency,
     ...
     reports_this as (select pcon20nm                                as constituency,
                             coalesce(sum(edm.spill_count), 0)       as total_spills,
                             coalesce(sum(edm.total_spill_hours), 0) as total_hours,
                             count(*)                                as cso_count
                      from edm_consent_view edm
     ...
     wanted as (select con.pcon20nm                                                                       as constituency,
                ...
                from pcon_dec_2020_uk_bfc con
                         left join reports_this on reports_this.constituency = con.pcon20nm
